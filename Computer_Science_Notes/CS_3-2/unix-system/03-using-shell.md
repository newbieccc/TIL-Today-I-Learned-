...
| 셀 예약어 | 예약된 단어로 do, while, case 등       |
| 함수    | 셀에서 수행되는 함수의 정의                 |
| 내장 명령 | 셀 내부에 존재하는 명령으로 cd, echo, pwd 등 |
...
| ↑ 또는 ↓            | 누를 때마다 직전 또는 직후 명령을 불러옴. 편집 가능 |
...
- 명령 행 오나선 기능
    ...
    - 정보가 충분하지 않은 경우, 탭 키를 한번 더 누르면 모든 가능한 경우를 보여주고 원래의 명령 행이 유지됨
...
| 문자          | 의미                                                      |
|-------------|---------------------------------------------------------|
| 백슬래시 (`\`)  | 특수 문자의 기능을 제거하는 이스케이프(escape) 문자 또는 긴 명령 행을 분리할 때 사용    |
...
| 문자               | 의미                                  |
|------------------|-------------------------------------|
| 앰퍼샌드 (`&`)       | 명령을 백그라운드로 실행시킴                     |
| 애스터리스크 (`*`)     | 파일 이름에서 사용할 때 0개 이상의 임의 문자열과 대응됨    |
| 물음표 (`?`)        | 파일 이름에서 사용할 때 1개 문자와 대응됨            |
| 파이프 (`      ㅣ `) | 앞 명령의 출력을 다음 명령의 입력으로 연결시킴          |
| `<` 또는 `>`       | 입·출력 리다이렉션에서 사용(파일로부터 입력받거나 파일로 출력) |
| `>>`             | 표준 출력을 파일의 끝에 덧붙일 때 사용(append)      |
...
| 파일                | 설명                                                                                  |
|-------------------|-------------------------------------------------------------------------------------|
| `/etc/profile`    | **모든 사용자**에게 적용. **로그인할 때** 가장 먼저 수행됨. `PATH`, `MAIL`, `HISTSIZE` 등 전역 환경 변수를 설정.   |
| `~/.bash_profile` | **로그인할 때** 수행되며 **개별 사용자**에게 적용됨. 보통 내부에서 `~/.bashrc`를 실행.                          |
| `~/.bashrc`       | **셸을 시작할 때마다** 수행되며 **개별 사용자**에게 적용됨. 보통 내부에서 `/etc/bashrc`를 수행.                    |
| `/etc/bashrc`     | **모든 사용자**에게 적용. **셸을 시작할 때마다** 수행됨. `PS1` 설정, 별칭(alias) 설정, `umask` 초기값, 함수 등을 설정. |
| `~/.bash_logout`  | **로그아웃**할 때 실행되는 스크립트 파일.                                                           |
